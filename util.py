...
class LinearWarmupCosineDecay:
    def __init__(
        ...
        #phase_map = {'linear': anneal_linear}
        ...
        # self.lr_phase = Phase(lr_min, lr_max, phase1, cur_iter_phase1, anneal_linear)

        # self.lr_phase = Phase(lr_max, lr_min / 1e4, phase2, cur_iter_phase2, anneal_cosine)
            #Phase(lr_max, lr_min / 1e4, phase2, cur_iter_phase2, phase_map[phase[1]]),
...
def info_nce_loss(eeg_features, img_features, tau=0.1):
    ...
    for i in range(batch_size):
        ...
        neg_logits[i] = torch.sum(neg_similarity)  
...
def loss_contrast(net, X, mrstftloss, **kwargs):
    """
    ...
    loss = 0.0
    denoised_audio, eeg_feature, audio_mask = net(noisy_audio, eeg)
    sc_loss = mrstftloss(denoised_audio.squeeze(1), clean_audio.squeeze(1))
    
    info_nce = 0.0
    
    info_nce = info_nce_loss(eeg_feature, audio_mask)
 
    
    total_loss = sc_loss + 3*info_nce
